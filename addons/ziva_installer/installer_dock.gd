...
enum ZivaState { IDLE, DOWNLOADING, EXTRACTING, COMPLETE, ERROR }

var _state: ZivaState = ZivaState.IDLE
...
func get_state() -> String:
	match _state:
		ZivaState.IDLE:
			return "idle"
		ZivaState.DOWNLOADING:
			return "downloading"
		ZivaState.EXTRACTING:
			return "extracting"
		ZivaState.COMPLETE:
			return "complete"
		ZivaState.ERROR:
...
func _on_state_changed(state_str: String, progress: float) -> void:
	...
	match state_str:
		"idle":
			_state = ZivaState.IDLE
		"downloading":
			_state = ZivaState.DOWNLOADING
			_install_button.visible = false
			_progress_bar.visible = true
			_progress_bar.value = progress * 100.0
			_status_label.text = ""
		"extracting":
			_state = ZivaState.EXTRACTING
			_progress_bar.visible = true
			_progress_bar.value = 100
			_status_label.text = "Extracting..."
		"complete":
			_state = ZivaState.COMPLETE
			_progress_bar.visible = false
